...
const ArticleList: React.FC<ArticleListProps> = ({ posts }) => {
  return (
    ...
    >
      {posts.map((post) => (
        <motion.div key={post.id} variants={itemAnimation} className="h-full">
          <Link 
            to={`/articles/${post.slug}`} 
            className="block group bg-gradient-to-br from-[#121212] to-[#0A0A0A] rounded-2xl overflow-hidden shadow-lg h-full flex flex-col transition-all duration-300 hover:shadow-xl hover:shadow-[#3CAAFF]/20 hover:scale-[1.02] border border-gray-800/30"
          >
            {/* Image Container */}

...
const Hero = () => {
  ...
  return (
    <div className="relative min-h-[90vh] md:min-h-screen overflow-hidden" ref={ref}>
      {/* Background with gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#0A0A0A]/50 via-[#111111]/30 to-[#0A0A0A]/50 z-30">
        <Suspense fallback={<div className="absolute inset-0 bg-[#0A0A0A]" />}>
          {!isMobile && <ClientThreeBackground />}
        </Suspense>
      </div>

      {/* Content */}
      <div className="relative z-40 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 md:pt-32 pb-12 md:pb-16">
        <div className="text-center lg:text-left lg:grid lg:grid-cols-2 lg:gap-8 items-center">
          ...
          >
            ...
            {/* Main heading */}
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight relative">
            ...
            {/* Description */}
            <p className="text-base md:text-lg text-[#ABABAB] max-w-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <button
                ...
                className="inline-flex items-center px-6 sm:px-8 py-3 rounded-full bg-gradient-to-r from-[#3CAAFF] to-[#00E0FF] text-[#0A0A0A] font-medium hover:shadow-lg hover:shadow-[#3CAAFF]/25 transition-all duration-300"
              >
                Contact Us
              ...
              <button
                ...
                className="inline-flex items-center px-6 sm:px-8 py-3 rounded-full border border-[#3CAAFF]/30 hover:border-[#3CAAFF]/60 text-[#ABABAB] hover:text-[#F5F5F7] transition-colors duration-300"
              >
                Explore Services
              </button>
            </div>
            ...
            <div className="pt-6 md:pt-8 border-t border-[#222222]/30">
              <div className={`${isMobile ? 'grid grid-cols-1 gap-6' : 'flex items-center justify-start space-x-8'}`}>
                <div>
                  <div className="flex items-center justify-center md:justify-start">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className="w-4 md:w-5 h-4 md:h-5 text-[#3CAAFF] fill-current" />
                    ))}
                  </div>
                  <p className="mt-2 text-sm text-[#777777]">Trusted by 100+ equestrians</p>
                </div>
                {!isMobile && <div className="h-12 w-px bg-[#222222]/30" />}
                <div>
          ...
          {/* Hero image/illustration - Simplified for mobile */}
          <div className="mt-8 lg:mt-0 relative">
            <div className="relative mx-auto max-w-[500px]">
              <div className="aspect-w-5 aspect-h-3 rounded-2xl overflow-hidden bg-gradient-to-br from-[#3CAAFF]/10 to-[#00E0FF]/10">
                <img
                  src={image}
                  alt="Elegant Horse"
